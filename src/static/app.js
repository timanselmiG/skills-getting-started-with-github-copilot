document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear loading message and repopulate activity select
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.length
          ? details.participants
              .map(
                (email) =>
                  `<li><span>${email}</span><button type="button" class="remove-participant" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">✕</button></li>`
              )
              .join("")
